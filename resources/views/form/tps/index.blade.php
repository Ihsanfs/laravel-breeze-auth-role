            ...
            <div class="col-md-4">
                <div class="card">
                    <div class="card-body">
                        ...
                        <ul>
                            <li class="bg-warning rounded p-2 text-dark mt-2 mb-2"> Jumlah sekretaris :
                                {{ $tpssek->count() }}</li>
                            <li class="bg-success rounded p-2 text-dark mt-2 mb-2"> Jumlah TPS :
                                {{ $tpssek->groupBy('tps')->count() }}</li>

                        </ul>
                        @php
                            $groupedsekretaris = $tpssek->groupBy('tps')->sort(function ($a, $b) {
                                return $a->first()->tps <=> $b->first()->tps;
                            });
                        @endphp

                        @foreach ($groupedsekretaris as $key => $group)
                            <ul>
                                ...
                                <div class="card-body bg-info rounded">
                                    @foreach ($group as $sekretarisItem)
                                        <li class="text-dark">
                                            <strong> Nama sekretaris : {{ $sekretarisItem->nama_gelar }}</strong>

                                            @php
                                                $totalTpsSekretaris = $semuatps->where('tps', $key)->count();
                                            @endphp

                                            <p> Total TPS: {{ $totalTpsSekretaris }}</p>
                                        </li>
                                    @endforeach
                                </div>
                            </ul>
